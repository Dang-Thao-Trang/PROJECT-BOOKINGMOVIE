...
const MovieBox = () => {
  ...
  const items = cinemas?.map((cinebox) => {
    const subItems = cinebox.lstCumRap
      ?.slice(0, 10)
      .map((cineComplex, index) => {
        return {
          label: (
            <div className="cine-info">
              <h3 className="cine-name">{cineComplex.tenCumRap}</h3>
              <p className="cine-address">{cineComplex.diaChi}</p>
              <span className="cine-detail">Chi Tiết</span>
            </div>
          ),
          key: index,
          children: cineComplex.danhSachPhim.map((film, index) => {
            return (
              <div key={index} className="cine-flim">
                ...
                <div className="cine-title">
                  <h1>
                    {film.hot && <span className="cine-sub">Hot</span>}
                    {film.tenPhim}
                  </h1>
                    ...
                    .map((showtimes, idx) => {
                      return (
                        <button
                          ...
                          className="cine-date"
                          onClick={() =>
                            navigate(`/ticket/${showtimes.maLichChieu}`)
                          }
                        >
                      ...
                      .map((cinemaComplex, idx) => {
                        return (
                          <Panel
                            ...
                            key={idx + 1}
                          >
                            {cinemaComplex.danhSachPhim.map(
                              (film, filmIndex) => {
                                return (
                                  <div
                                    key={filmIndex}
